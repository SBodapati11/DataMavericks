...
# Read the play by play data and player data
...
# Get the names and and nba team ids of all the teams
...
# Read the lineup, offesnive score, and defensive score data
...
# Get all the playing positions of a given player
...
def Lineups():
    ...
    selected_team = st.selectbox("Select an opponent to generate lineups", teams, index=0)
    st.markdown("Optimal lineups against: **{0}**".format(selected_team))

    team_abbrev = get_team_abbrev(int(get_team_id(selected_team)))

    # Get the generated lineups for the selected team and restructure the dataset
    ...
    for i in range(len(team_lineups.index)):
        ...
        # Get all the positions for every player in the current lineup
        ...
        # Find all combinations of the lineup with every position
        all_combinations = {}
        count = 0
        ...
        # Remove all combinations which has the same player in multiple positions
        ...
        # Create a lineup and get the scores for every combination
        for key in all_combinations:
            ...
            offense_strength, defense_strength, offense_count, defense_count = 0, 0, 0, 0
            # Calculate the offensive and defensive scores
    ...
    play_col.metric(":green[**Average Play Type**]", "Offense")
