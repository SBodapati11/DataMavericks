...
def SeasonDataFiltering():
    st.header("🗑️ Player Shot Analysis")
    ...
    with st.expander(label="**Filter the season data by a specific player**"):
        with st.form("PlayerShotsForm", clear_on_submit=True):
            ...
            if player_shots_submitted:
                ...
                player_pbp = mavs_pbp_season[(mavs_pbp_season['playerId1'] == player.at[0, 'nbaId']) | (
                            mavs_pbp_season['playerId2'] == player.at[0, 'nbaId']) | (
                                                         mavs_pbp_season['playerId3'] == player.at[
                                                     0, 'nbaId'])].reset_index()
                ...
                x_values, y_values = np.mgrid[
                                     shots_df['LOC_X'].min():shots_df['LOC_X'].max():shots_df['LOC_X'].size ** 0.5 * 1j,
